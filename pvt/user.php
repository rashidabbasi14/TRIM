...
<script>
	...
	<body>
			<div class="panel panel-default" style="position:relative;">
			  <div class="panel-body">
					<button name="addField" id="addField" class ="btn btn-default" onclick="adduser();"> Add User </button>
					<button name="RemItem" id="RemItem" class ="btn btn-default" onclick="test();"> Remove User </button>
			  </div>
			</div>
					...
					<?php
						...
						while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_NUMERIC))  {
							...
							echo "<td class='cToggle' style='display:none;'><input value='".$row[0]."' class='citem' type='checkbox' style='position:relative;'></td>";
							 ...
							 echo "<td> ".$row[1]."</td>"; 
							 echo "<td> ".$row[2]."</td>"; 
							 echo "<td> ".date_format($row[3], 'Y-m-d')."</td>";
							 echo "<td> ".($row[4] == 1 ? "Admin" : "Standard")."</td>";
							 echo "<td> ".$row[5]."</td>";
							echo "</tr>";
						}  
					?>
	...
	<script>
		...
		{ 
			...
			var url = "phpdb/delUserFromDB.php";
